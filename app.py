...
# making sure that upload folder exists
os.makedirs(UPLOAD_FOLDER, exist_ok=True)


def allowed_file(filename):
    """Checking here if the uploaded file is an allowed image type."""
...
def home():
    """Welcome route."""
    return "Welcome to the Image Upload and Description API for - [ CC Yard Private Limited ]"


@app.route('/upload', methods=['POST'])
def upload_image():
    """Endpoint to upload an image and generate descriptions."""
    # Checking if file is part of the request
    if 'file' not in request.files:
        return jsonify({"error": "No file selected for upload"}), 400

    file = request.files['file']

    # Checking if a file is selected and has a valid extension
    if file.filename == '':
        return jsonify({"error": "No file selected for upload"}), 400
